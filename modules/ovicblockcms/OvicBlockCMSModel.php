...
class OvicBlockCMSModel extends ObjectModel
{
	public $id_cms_block;
	...
	{
		return (
			OvicBlockCMSModel::createCMSBlockTable() &&
			OvicBlockCMSModel::createCMSBlockLangTable() &&
			OvicBlockCMSModel::createCMSBlockPageTable() &&
			OvicBlockCMSModel::createCMSBlockShopTable()
		);
	}
	...
	public static function getCMSTitlesFooter()
	{
		...
		foreach ($cmsCategories as $cmsCategory)
		{
			...
			{
				$query = OvicBlockCMSModel::getBlockName($ids[1]);
			...
			{
				$query = OvicBlockCMSModel::getCMSMetaTitle($ids[1]);
	...
	public static function getCMSCategories($recursive = false, $parent = 0)
	{
		...
		else
		{
			...
			{
				$sub_categories = OvicBlockCMSModel::getCMSCategories(true, $result['id_cms_category']);
	...
	{
		$categories = OvicBlockCMSModel::getCMSCategories();
        $id_shop = ($id_shop !== false) ? $id_shop : Context::getContext()->shop->id;

		foreach ($categories as $key => $value)
			$categories[$key]['cms_pages'] = OvicBlockCMSModel::getCMSPages($value['id_cms_category'], $id_shop);
	...
	{
		$titles = array_merge(
			OvicBlockCMSModel::getCMSTitles(OvicBlockCMSModel::LEFT_COLUMN),
			OvicBlockCMSModel::getCMSTitles(OvicBlockCMSModel::RIGHT_COLUMN),
            OvicBlockCMSModel::getCMSTitles(OvicBlockCMSModel::CMS_POS)
		);
