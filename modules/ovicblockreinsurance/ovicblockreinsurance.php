...
include_once _PS_MODULE_DIR_.'ovicblockreinsurance/ovicReinsuranceClass.php';

class ovicBlockReinsurance extends Module
{
	public function __construct()
	{
		$this->name = 'ovicblockreinsurance';
		...
		$this->displayName = $this->l('Ovic - Customer reassurance block');
	...
	public function install()
	{
		return parent::install() &&
			$this->installDB() &&
			Configuration::updateValue('OVIC_BLOCKREINSURANCE_NBBLOCKS', 5) &&
			$this->registerHook('ovichookreinsurance') && $this->installFixtures() &&
	...
	public function installDB()
	{
		...
		$return &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'ovicreinsurance` (
		...
		$return &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'ovicreinsurance_lang` (
				`id_reinsurance` INT UNSIGNED NOT NULL AUTO_INCREMENT,
	...
	public function uninstall()
	{
		...
		return Configuration::deleteByName('OVIC_BLOCKREINSURANCE_NBBLOCKS') &&
			$this->uninstallDB() &&
			parent::uninstall();
	}
	...
	{
		return Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'ovicreinsurance`') && Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'ovicreinsurance_lang`');
	}
	...
	public function addToDB()
	{
		if (isset($_POST['nbblocks']))
		{
			for ($i = 1; $i <= (int)$_POST['nbblocks']; $i++)
			{
				...
				Db::getInstance()->execute('INSERT INTO `'._DB_PREFIX_.'ovicreinsurance` (`filename`,`text`,`text2`,`text3`)
											VALUES ("'.((isset($filename[0]) && $filename[0] != '') ? pSQL($filename[0]) : '').
					'", "'.((isset($_POST['info'.$i.'_text']) && $_POST['info'.$i.'_text'] != '') ? pSQL($_POST['info'.$i.'_text']) : '').'", "'.((isset($_POST['info'.$i.'_text2']) && $_POST['info'.$i.'_text2'] != '') ? pSQL($_POST['info'.$i.'_text2']) : '').'", "'.((isset($_POST['info'.$i.'_text3']) && $_POST['info'.$i.'_text3'] != '') ? pSQL($_POST['info'.$i.'_text3']) : '').'")');
			}
			return true;
		} else
			return false;
	...
	public function removeFromDB()
	{
		...
		return Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'ovicreinsurance`');
	}
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('saveovicblockreinsurance'))
		{
			if ($id_reinsurance = Tools::getValue('id_reinsurance'))
				$reinsurance = new ovicReinsuranceClass((int)$id_reinsurance);
			else
				$reinsurance = new ovicReinsuranceClass();
			...
			if ($reinsurance->validateFields(false) && $reinsurance->validateFieldsLang(false))
			{
				...
				$this->_clearCache('ovicblockreinsurance.tpl');
			}
			else
				$html .= '<div class="conf error">'.$this->l('An error occurred while attempting to save.').'</div>';
		}
		
		if (Tools::isSubmit('updateovicblockreinsurance') || Tools::isSubmit('addovicblockreinsurance'))
		{
			...
			foreach (Language::getLanguages(false) as $lang)
				if ($id_reinsurance)
				{
                    ...
                    $helper->fields_value['text2'][(int)$lang['id_lang']] = $reinsurance->text2[(int)$lang['id_lang']];
                    $helper->fields_value['text3'][(int)$lang['id_lang']] = $reinsurance->text3[(int)$lang['id_lang']];
				}	
				else{
				    $helper->fields_value['text'][(int)$lang['id_lang']] = Tools::getValue('text_'.(int)$lang['id_lang'], '');
                    $helper->fields_value['text2'][(int)$lang['id_lang']] = Tools::getValue('text2_'.(int)$lang['id_lang'], '');
                    $helper->fields_value['text3'][(int)$lang['id_lang']] = Tools::getValue('text3_'.(int)$lang['id_lang'], '');
				}
		...
		else if (Tools::isSubmit('deleteovicblockreinsurance'))
		{
			$reinsurance = new ovicReinsuranceClass((int)$id_reinsurance);
		...
		{
			Configuration::updateValue('OVIC_BLOCKREINSURANCE_NBBLOCKS', ((isset($_POST['nbblocks']) && $_POST['nbblocks'] != '') ? (int)$_POST['nbblocks'] : ''));
	...
	protected function getListContent($id_lang)
	{
		return  Db::getInstance()->executeS('
			SELECT r.`id_reinsurance`, r.`id_shop`, r.`file_name`, rl.`text`,  rl.`text2`, rl.`text3`
			FROM `'._DB_PREFIX_.'ovicreinsurance` r
			LEFT JOIN `'._DB_PREFIX_.'ovicreinsurance_lang` rl ON (r.`id_reinsurance` = rl.`id_reinsurance`)
	...
	protected function initForm()
	{
		...
		$helper->module = $this;
		$helper->name_controller = 'ovicblockreinsurance';
		...
		$helper->submit_action = 'saveovicblockreinsurance';
	...
	public function hookOvichookreinsurance($params)
	{
		//$this->context->controller->addCSS($this->_path.'style.css', 'all');
		if (!$this->isCached('ovicblockreinsurance.tpl', $this->getCacheId()))
		{
			$infos = $this->getListContent($this->context->language->id);
			$this->context->smarty->assign(array('infos' => $infos, 'nbblocks' => count($infos)));
		}
		return $this->display(__FILE__, 'ovicblockreinsurance.tpl', $this->getCacheId());
	}
	...
	public function installFixtures()
	{
		$return = true;
		$tab_texts = array(
			array('text' => $this->l('Free Shipping'), 'text2' => $this->l('Sollicitudin mauris cursus'), 'text3' => $this->l('On orders over $99'), 'file_name' => 'reinsurance-1-1.png'),
			array('text' => $this->l('Free Returns'),  'text2' => $this->l('Sollicitudin mauris cursus'), 'text3' => $this->l('On all orders'), 'file_name' => 'reinsurance-2-1.png')
		);
		...
		foreach($tab_texts as $tab)
		{
			...
			foreach (Language::getLanguages(false) as $lang){
                 ...
                 $reinsurance->text2[$lang['id_lang']] = $tab['text2'];
                 $reinsurance->text3[$lang['id_lang']] = $tab['text3'];
			}
