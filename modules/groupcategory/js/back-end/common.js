...
function groupCategory_HandleEnterNumber(event){
...
function groupCategory_HandleEnterNumberInt(event){
	var keyCode = event.keyCode ? event.keyCode : event.charCode;
...
function saveGroup(){
	...
	var manufacturerIds = [];
    $('#manufacturerIds option:selected').each(function(i){ 
       manufacturerIds[i] = $(this).val();
    });
   	data.manufacturerIds = manufacturerIds;
    $.ajax({
		type:'POST',
		url: baseModuleUrl + '/ajax.php',
		data: data,
