...
class VerticalMegaMenus extends Module
{
	...
	*/
    ...
    public function hooks($hookName, $param){
        ...
        if (!$this->isCached($moduleLayout, $hookId.'-'.$pageCache)){
            ...
            if($items){            
                foreach($items as $i=>$item){                    			
                    ...
                    //$this->clearCache();                                
                    //if (!$this->isCached($moduleLayout, $this->getCacheId())){
                	$modules[$i]['title'] = $item['name'];
                	$modules[$i]['layout'] = $item['layout'];
                    $modules[$i]['sections'] = $this->$buildFunction($item, $langId, $shopId);                                            
            		//}
                }			  
            }   
            $this->context->smarty->assign('verticalModules', $modules); 
        }        
        return $this->display(__FILE__, $moduleLayout, $hookId.'-'.$pageCache);    
    }
    function buildModule_default($module, $langId, $shopId){    	
    	//if (!$this->isCached('menu.tpl', 'menu|'.$module['id'])){
    		...
    		if($menus){
    			foreach ($menus as $i => &$menu) {
	              if($menu['menuType'] == 'link')
    				...
    				$menu['iconPath'] = $icon;
    				$menu['group_content'] = $this->buildGroups_default($menu, $langId, $shopId);
    			}
    		}
    		$this->context->smarty->assign(array(
    			'verticalMenus'=>$menus,
        ...
        //}
		return $this->display(__FILE__, 'menu.tpl', 'menu|'.$module['id']);
		
    	//return $html;
    }
	function buildGroups_default($menu, $langId, $shopId){    	
    	//if (!$this->isCached('group.tpl', 'group|'.$menu['id'])){
    		...
    		if($groups){
    			foreach ($groups as $i => &$group) {				
    				if($group['type'] == 'custom') $group['group_content'] = $this->buildGroup_ItemCustom_default($group, $langId, $shopId);
    				elseif($group['type'] == 'link') $group['group_content'] = $this->buildGroup_ItemLink_default($group, $langId, $shopId);
    				else $group['group_content'] = $this->buildGroup_Products_default($group, $langId, $shopId);				
    			}
    		}		
    		$this->context->smarty->assign(array(
		...
		return $this->display(__FILE__, 'group.tpl', 'group|'.$menu['id']);
		//return $html;
    }
    		...
    		if($products){            
                foreach($products as $i => &$product){
                	if(isset(self::$productCache['prod-'.$product['id_product']]) && self::$productCache['prod-'.$product['id_product']]){
    	                ...
    	                if(strpos($imagePath, $http) === false) $imagePath = $http.$imagePath;    
    ...
    function buildGroup_ItemLink_default($group, $langId, $shopId){
    	//if (!$this->isCached('group.link.tpl', 'link|'.$group['id'])){
    	   $params = json_decode($group['params']);    	
		...
		return $this->display(__FILE__, 'group.link.tpl', 'link|'.$group['id']);
    }
	function buildGroup_ItemCustom_default($group, $langId, $shopId){    	
    	//if (!$this->isCached('group.custom.tpl', 'custom|'.$group['id'])){
    	   $params = json_decode($group['params']);    	
		...
		return $this->display(__FILE__, 'group.custom.tpl', 'custom|'.$group['id']);
    }
