...
<div id="layered_block_left" class="block">
	<p class="title_block">{l s='Filter selection' mod='blocklayered'}</p>
			...
			<div>
				...
				<div id="enabled_filters">
					<span class="layered_subtitle" style="float: none;">
						{l s='YOU ARE VIEWING' mod='blocklayered'}
					</span>
					<ul>
                    ...
                    <a href="javascript:void(0)" class="remove-all">{l s ='Remove all' mod='blocklayered'}</a>
				</div>
				{/if}
				{foreach from=$filters item=filter}
					{if isset($filter.values)}
						...
						{else}
							<div class="layered_filter{if isset($filter.is_color_group) && $filter.is_color_group} color_class{/if} {if $filter.type == 'id_attribute_group'} size_class{/if}{if $filter.type == 'manufacturer'} manufacturer_class{/if}">
						{/if}
									...
									{foreach from=$filter.values key=id_value item=value name=fe}
										...
										<li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if} col-lg-6">
											{if isset($filter.is_color_group) && $filter.is_color_group}
												<input class="color-option {if isset($value.checked) && $value.checked}on{/if} {if !$value.nbr}disable{/if}" value="{$value.color}" type="button" name="layered_{$filter.type_lite}_{$id_value}" data-rel="{$id_value}_{$filter.id_key}" id="layered_id_attribute_group_{$id_value}" {if !$value.nbr}disabled="disabled"{/if} style="background: {if isset($value.color)}{if file_exists($smarty.const._PS_ROOT_DIR_|cat:"/img/co/$id_value.jpg")}url(img/co/{$id_value}.jpg){else}{$value.color}{/if}{else}#CCC{/if};" />
											...
											{/if}
											<label for="layered_{$filter.type_lite}_{$id_value}"{if !$value.nbr} class="disabled"{else}{if isset($filter.is_color_group) && $filter.is_color_group} {*}name="layered_{$filter.type_lite}_{$id_value}"{*} class="layered_color {if isset($value.checked) && $value.checked}on{/if}" data-rel="{$id_value}_{$filter.id_key}"{/if}{/if}>
												{if !$value.nbr}
												{$value.name|escape:'html':'UTF-8'}{if $layered_show_qties}<span> ({$value.nbr})</span>{/if}
												{else}
												<a href="{$value.link}"{if $value.rel|trim != ''} data-rel="{$value.rel}"{/if}>{if (($filter.type == 'id_attribute_group') && (isset($filter.is_color_group) && !$filter.is_color_group))} <span class="size_span">{/if}{$value.name|escape:'html':'UTF-8'}{if (($filter.type == 'id_attribute_group') && (isset($filter.is_color_group) && !$filter.is_color_group))} </span>{/if}{if $layered_show_qties}<span> ({$value.nbr})</span>{/if}</a>
												{/if}
											</label>
										</li>
										{/if}
