...
OvicFlexibleBrandsAjax::$action();
class OvicFlexibleBrandsAjax{
		...
		if($itemId == 0){
            ...
            if($db->execute("Insert Into "._DB_PREFIX_."flexiblebrand_modules (`id_shop`, `position`, `position_name`, `status`, `params`, `ordering`) Values ('".$shopId."', '$position', '$position_name', 1, '".json_encode($params)."', '$maxOrdering')")){
                ...
                if($languages){
					...
					if($insertSql) $db->insert('flexiblebrand_module_lang', $insertSql);
                }                
                $response->status = 1;
	...
	static function saveGroup(){
		...
		$response = new stdClass();
        if($itemId == 0){
        	$maxOrdering = $db->getValue("Select MAX(ordering) From "._DB_PREFIX_."flexiblebrand_groups Where `module_id` = ".$moduleId);
			if($maxOrdering >0) $maxOrdering++;
			else $maxOrdering = 1;
            if($db->execute("Insert Into "._DB_PREFIX_."flexiblebrand_groups (`module_id`, `manufacturerId`, `productCount`, `maxItem`, `type`, `ordering`, `status`, `params`) Values ('$moduleId', '$manufacturerId', '$productCount', '$maxItem', '$type', '$maxOrdering', '1', '".json_encode($params)."')")){
                $insertId = $db->Insert_ID();
				if($icon && file_exists($module->tempPath.$icon)){
                    ...
                    if(copy($module->tempPath.$icon, $module->iconPath.$fileName)){
                    	...
                    	DB::getInstance()->execute("Update "._DB_PREFIX_."flexiblebrand_groups Set icon = '$fileName' Where `id` = ".$insertId);	
                    }                    
                }
                if($iconActive && file_exists($module->tempPath.$iconActive)){
					$path_info = pathinfo($iconActive);
                    $ext = $path_info['extension'];
                    $fileName = $insertId.'-icon-active.'.$ext;
                    if(copy($module->tempPath.$iconActive, $module->iconPath.$fileName)){
                    	unlink($module->tempPath.$iconActive);
                    	DB::getInstance()->execute("Update "._DB_PREFIX_."flexiblebrand_groups Set iconActive = '$fileName' Where `id` = ".$insertId);	
                    }                    
                }
				if($languages){
                	$insertSql = array();
					...
					if($insertSql) $db->insert('flexiblebrand_group_lang', $insertSql);                
                }
				$response->status = 1;
            ...
            $item = $db->getRow("Select * From "._DB_PREFIX_."flexiblebrand_groups Where id = ".$itemId);			
            if($item){            	
                $db->execute("Update "._DB_PREFIX_."flexiblebrand_groups Set `manufacturerId` = ".$manufacturerId.", `type`='$type', `productCount`='$productCount', `maxItem`='$maxItem', `params`='".json_encode($params)."' Where id = ".$itemId);
				if($icon && file_exists($module->tempPath.$icon)){
					$path_info = pathinfo($icon);
                    $ext = $path_info['extension'];
                    ...
                    if(copy($module->tempPath.$icon, $module->iconPath.$fileName)){
                    	unlink($module->tempPath.$icon);
                    	$db->execute("Update "._DB_PREFIX_."flexiblebrand_groups Set  `icon`='$fileName' Where id = ".$itemId);	
                    }               
                }
				...
				if($iconActive && file_exists($module->tempPath.$iconActive)){
                    ...
                    if(copy($module->tempPath.$iconActive, $module->iconPath.$fileName)){
                    	unlink($module->tempPath.$iconActive);
                    	$db->execute("Update "._DB_PREFIX_."flexiblebrand_groups Set  `iconActive`='$fileName' Where id = ".$itemId);	
                    }               
                }
				...
				if($languages){
                	$insertSql = array();
                	foreach($languages as $i=>$language){
                		$check = DB::getInstance()->getValue("Select group_id From "._DB_PREFIX_."flexiblebrand_group_lang Where `group_id` = '$itemId' AND `id_lang` = '".$language->id."' AND `id_shop` = '".$shopId."'");
						if($check){
							DB::getInstance()->execute("Update "._DB_PREFIX_."flexiblebrand_group_lang Set `title` = '".$db->escape($titles[$i])."' Where `group_id` = '$itemId' AND `id_lang` = '".$language->id."' AND `id_shop` = '".$shopId."'");
						}else{
	...
	static function updateModuleOrdering(){
	   $module = new  FlexibleBrands();
        ...
        if($ids){
            $strIds = implode(', ', $ids);            
            $minOrder = DB::getInstance()->getValue("Select Min(ordering) From "._DB_PREFIX_."flexiblebrand_modules Where id IN ($strIds)");            
            foreach($ids as $i=>$id){
                DB::getInstance()->query("Update "._DB_PREFIX_."flexiblebrand_modules Set ordering=".($minOrder + $i)." Where id = ".$id);                
            }
        }
	...
	static function updateGroupOrdering(){
	   $module = new  FlexibleBrands();
        ...
        if($ids){
            $strIds = implode(', ', $ids);            
            $minOrder = DB::getInstance()->getValue("Select Min(ordering) From "._DB_PREFIX_."flexiblebrand_groups Where id IN ($strIds)");            
            foreach($ids as $i=>$id){
                DB::getInstance()->query("Update "._DB_PREFIX_."flexiblebrand_groups Set ordering=".($minOrder + $i)." Where id = ".$id);                
            }
        }
        die(Tools::jsonEncode('Update ordering success!'));
    }
	...
	static function loadProductByGroup(){
        ...
        $group = DB::getInstance()->getRow("Select * From "._DB_PREFIX_."flexiblebrand_groups Where id = ".$groupId);
        $module = DB::getInstance()->getRow("Select * From "._DB_PREFIX_."flexiblebrand_modules Where id = ".$moduleId);
        ...
        if($group['type'] == 'auto'){
            if($params->orderValue == 'sales'){
                $products =  $flexModule->getProductsOrderSales($langId, $group['manufacturerId'], $params, $group['maxItem'], true, false, false, 0);                
                ...
                $products = $flexModule->getProductsOrderPrice($langId, $group['manufacturerId'], $params, $group['maxItem'], true, false, false, 0);
               ...
               //$total = Functions::getProductsOrderSpecial($langId, $arrSubCategory, $params, $moduleCategoryItem['productCount'], true, false, true);
                $products = $flexModule->getProductsOrderSpecial($langId, $group['manufacturerId'], $params, $group['maxItem'], true, false, false, 0);
               ...
               //$total = Functions::getProductsOrderAddDate($langId, $arrSubCategory, $params, $moduleCategoryItem['productCount'], true, false, true);
                $products = $flexModule->getProductsOrderAddDate($langId, $group['manufacturerId'], $params, $group['maxItem'], true, false, false, 0);
            }elseif($params->orderValue == 'rand'){
                //$total = $moduleCategoryItem['productCount'];
                $products = $flexModule->getProductsOrderRand($langId, $group['manufacturerId'], $params, $group['maxItem'], true, false);
            }            
            if($products){
            ...
            $items = DB::getInstance()->executeS("Select product_id, ordering From "._DB_PREFIX_."flexiblebrand_group_products Where module_id = ".$moduleId." AND group_id = ".$groupId." Order By ordering");
            if($items){                
                foreach($items as $item){                    
                    $product = $flexModule->getProductById($langId, $item['product_id'], true, false);              
                    $imagePath = $link->getImageLink($product['link_rewrite'], $product['id_image'], 'cart_default');                        
                    $response->content .= '<tr id="ptr_'.$product['id_product'].'">
                                <td>'.$product['id_product'].'</td>
                                <td class="center"><img src="'.$imagePath.'" width="60" /></td>
	...
	static function loadListProducts(){		
        ...
        $rows = DB::getInstance()->executeS("Select product_id From "._DB_PREFIX_."flexiblebrand_group_products Where module_id = ".$moduleId." AND `group_id` = ".$groupId);
        $arrProductId = array();
        $arrProductId[] = '0';
        if($rows){
        ...
        $group = DB::getInstance()->getRow("Select * From "._DB_PREFIX_."flexiblebrand_groups Where id = ".$groupId);        
        $offset=($page - 1) * $pageSize;
        $total = $fleModule->getProductList($langId, $group['manufacturerId'], $productIds, $keyword, true);		
		$response = new stdClass();
        $response->pagination = '';
        $response->list = '';
        if($total >0){            
            $response->pagination = $fleModule->paginationAjaxEx($total, $pageSize, $page, 6, 'loadListProducts');
            $items = $fleModule->getProductList($langId, $group['manufacturerId'], $productIds, $keyword, false, $offset, $pageSize);
            if($items){
                if($items){
                    foreach($items as $item){
	...
	static function addManualProductItem(){
	   $module = new  FlexibleBrands();
        ...
        $itemId = intval($_POST['itemId']);
        $moduleId = intval($_POST['moduleId']);
        $check = DB::getInstance()->getRow("Select product_id From "._DB_PREFIX_."flexiblebrand_group_products Where module_id = ".$moduleId." AND `group_id` = ".$groupId." AND product_id = ".$itemId);
        $response = new stdClass();
        if($check){
            $response->status = 1;
            $response->msg = 'Add product success!';            
        }else{
            $maxOrdering = (int)DB::getInstance()->getValue("Select MAX(ordering) From "._DB_PREFIX_."flexiblebrand_group_products Where module_id = ".$moduleId." AND group_id = ".$groupId);
            if(DB::getInstance()->execute("Insert Into "._DB_PREFIX_."flexiblebrand_group_products (`module_id`, `group_id`, `product_id`, `ordering`) Values ('".$moduleId."', '".$groupId."', '".$itemId."', '".($maxOrdering + 1)."')")){
                $response->status = 1;
                $response->msg = 'Add product success!';    
            }else{
                $response->status = 0;
    ...
    static function deleteManualProductItem(){
        ...
        $groupId = intval($_POST['groupId']);
        $moduleId = intval($_POST['moduleId']);
        if(DB::getInstance(_PS_USE_SQL_SLAVE_)->execute("Delete From "._DB_PREFIX_."flexiblebrand_group_products Where `module_id` = ".$moduleId." AND `group_id` = ".$groupId." AND `product_id` = ".$itemId)){
    ...
    static function deleteGroup(){
		...
		$response = new stdClass();
        $itemId = intval($_POST['itemId']);      
		$item = DB::getInstance()->getRow("Select * From "._DB_PREFIX_."flexiblebrand_groups Where id = $itemId");
		if($item){
			if(DB::getInstance()->execute("Delete From "._DB_PREFIX_."flexiblebrand_groups Where id = ".$itemId)){
				DB::getInstance()->escape("Delete From "._DB_PREFIX_."flexiblebrand_group_lang Where group_id = $itemId");
				if($item['icon'] && file_exists($module->iconPath.$item['icon'])) unlink($module->iconPath.$item['icon']);
				DB::getInstance()->escape("Delete From "._DB_PREFIX_."flexiblebrand_group_products Where group_id = $itemId");
				$response->status = 1;
	...
	static function updateProductOrdering(){
	   $module = new  FlexibleBrands();
        ...
        if($ids){
            $strIds = implode(', ', $ids);            
            $minOrder = DB::getInstance()->getValue("Select Min(ordering) From "._DB_PREFIX_."flexiblebrand_group_products Where module_id = $moduleId AND group_id = '$groupId' AND product_id IN ($strIds)");            
            foreach($ids as $i=>$id){
                DB::getInstance()->query("Update "._DB_PREFIX_."flexiblebrand_group_products Set ordering=".($minOrder + $i)." Where module_id = $moduleId AND group_id = '$groupId' AND product_id = ".$id);                
            }
        }
        die(Tools::jsonEncode('Update ordering success!'));
    }
	static function deleteModule(){
        ...
        $module->clearCache();
        $itemId = intval($_POST['itemId']);  
		$check = (int)DB::getInstance()->getValue("Select count(*) From "._DB_PREFIX_."flexiblebrand_groups Where module_id = $itemId");
		$response = new stdClass();
		if($check >0){
			$response->status = 0;
			...
			if(DB::getInstance()->execute("Delete From "._DB_PREFIX_."flexiblebrand_modules Where id = ".$itemId)){
				$itemLangs = DB::getInstance()->executeS("Select * From "._DB_PREFIX_."flexiblebrand_module_lang Where module_id = $itemId");
	            if($itemLangs){
	            ...
	            DB::getInstance()->execute("Delete From "._DB_PREFIX_."flexiblebrand_module_lang Where module_id = ".$itemId);
				$response->status = 1;
    ...
    public static function changeModuleStatus(){
		...
		$response = new stdClass();
		if($value == '1'){
			DB::getInstance()->execute("Update "._DB_PREFIX_."flexiblebrand_modules Set `status` = 0 Where id = ".$itemId);
			$response->status = 1;
			$response->msg = 'Update status success';
		}else{
			DB::getInstance()->execute("Update "._DB_PREFIX_."flexiblebrand_modules Set `status` = 1 Where id = ".$itemId);
			$response->status = 1;
			$response->msg = 'Update status success';
		}		
		die(Tools::jsonEncode($response));
    ...
    public static function changeGroupStatus(){
		...
		$response = new stdClass();
		if($value == '1'){
			DB::getInstance()->execute("Update "._DB_PREFIX_."flexiblebrand_groups Set `status` = 0 Where id = ".$itemId);
			$response->status = 1;
			$response->msg = 'Update status success';
		}else{
			DB::getInstance()->execute("Update "._DB_PREFIX_."flexiblebrand_groups Set `status` = 1 Where id = ".$itemId);
			$response->status = 1;
			$response->msg = 'Update status success';
		}		
		die(Tools::jsonEncode($response));
