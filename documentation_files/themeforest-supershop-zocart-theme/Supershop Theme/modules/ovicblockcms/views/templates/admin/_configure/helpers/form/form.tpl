...
{block name="input_row"}
					...
					<div class="col-lg-12">
						<div class="panel">
							<div class="panel-heading">
								{if $key == 0}
									{l s='Left blocks' mod='ovicblockcms'}
								{elseif $key == 1}
									{l s='Right blocks' mod='ovicblockcms'}
                                {else}
                                    {l s='CMS Position' mod='ovicblockcms'}
								{/if}
							</div>
							<table class="table tableDnD cms" id="cms_block_{$key%2}">
								<thead>
									<tr class="nodrag nodrop">
										<th>{l s='ID' mod='ovicblockcms'}</th>
										<th>{l s='Name of the block' mod='ovicblockcms'}</th>
										<th>{l s='Category name' mod='ovicblockcms'}</th>
										<th>{l s='Position' mod='ovicblockcms'}</th>
										<th></th>
									</tr>
								</thead>
								<tbody>
											...
											<td class="center pointer dragHandle" id="td_{$key%2}_{$cms_block['id_cms_block']}">
												<a
													{if (($key == (sizeof($cms_blocks_position) - 1)) || (sizeof($cms_blocks_position) == 1))}
															style="display: none;"
													{/if}
															href="{$current}&amp;configure=ovicblockcms&amp;id_cms_block={$cms_block['id_cms_block']}&amp;way=1&amp;position={(int)$cms_block['position'] + 1}&location=0&amp;token={$token}">
													<img src="{$smarty.const._PS_ADMIN_IMG_}down.gif" alt="{l s='Down' mod='ovicblockcms'}" title="{l s='Down' mod='ovicblockcms'}" />
												</a>
												<a
													{if (($cms_block['position'] == 0) || ($key == 0))}
															style="display: none;"
													{/if}
															href="{$current}&amp;configure=ovicblockcms&amp;id_cms_block={$cms_block['id_cms_block']}&amp;way=0&amp;position={(int)$cms_block['position'] - 1}&amp;location=0&amp;token={$token}">
													<img src="{$smarty.const._PS_ADMIN_IMG_}up.gif" alt="{l s='Up' mod='ovicblockcms'}" title="{l s='Up' mod='ovicblockcms'}" />
												</a>
											</td>
											<td>
												<div class="btn-group-action">
													<div class="btn-group pull-right">
														<a class="btn btn-default" href="{$current}&amp;token={$token}&amp;editBlockCMS&amp;id_cms_block={(int)$cms_block['id_cms_block']}" title="{l s='Edit' mod='ovicblockcms'}">
															<i class="icon-edit"></i> {l s='Edit' mod='ovicblockcms'}
														</a>
														...
														<li>
															<a href="{$current}&amp;token={$token}&amp;deleteBlockCMS&amp;id_cms_block={(int)$cms_block['id_cms_block']}" title="{l s='Delete' mod='ovicblockcms'}">
																<i class="icon-trash"></i> {l s='Delete' mod='ovicblockcms'}
															</a>
														</li>
														</ul>
													</div>
	...
	{elseif $input.type == 'cms_pages'}
		...
		{if isset($cms) && count($cms) > 0}
						...
						<table class="table">
							<thead>
								<tr>
									...
									<th>{l s='ID' mod='ovicblockcms'}</th>
									<th>{l s='Name' mod='ovicblockcms'}</th>
								</tr>
							</thead>
							<tbody>
			...
			{else}
			<p>{l s='No pages have been created.' mod='ovicblockcms'}</p>
		{/if}
	{else}
		{$smarty.block.parent}
	{/if}
