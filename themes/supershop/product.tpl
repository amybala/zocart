...
{if $errors|@count == 0}
	...
	{/if}
    {if isset($compared_products)}{addJsDef comparedProductsIds=$compared_products}{/if}
	...
	<div class="primary_block row">
		...
		<div class="pb-left-column col-xs-12 col-sm-4 col-md-5">
			...
			<div id="image-block" class="clearfix">
				...
				{/if}
				{if $product->on_sale}
					<span class="sale-box no-print{if !$product->new} no-new{/if}">
				...
				{if $have_image}
					<span id="view_full_size">
						{if $jqZoomEnabled && $have_image && !$content_only}
                            ...
                            {if !$content_only && $jqZoomEnabled}
								<span class="span_link no-print"><i class="fa fa-search"></i>&nbsp;{l s='Mouse over to zoom in'}</span>
							{/if}
						{else}
					...
					<span id="view_full_size">
						...
						{if !$content_only}
							<span class="span_link">
								<i class="fa fa-search"></i>&nbsp;{l s='Mouse over to zoom in'}
							</span>
						{/if}
					</span>
				{/if}
							...
							{foreach from=$images item=image name=thumbnails}
								...
								<li id="thumbnail_{$image.id_image}"{if $smarty.foreach.thumbnails.last} class="last"{/if}>
									<a{if $jqZoomEnabled && $have_image && !$content_only} href="javascript:void(0);" rel="{literal}{{/literal}gallery: 'gal1', smallimage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'large_default')|escape:'html':'UTF-8'}',largeimage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html':'UTF-8'}'{literal}}{/literal}"{else} href="{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html':'UTF-8'}"	data-fancybox-group="other-views" class="fancybox{if $image.id_image == $cover.id_image} shown{/if}"{/if} title="{$imageTitle}">
										<img class="img-responsive" id="thumb_{$image.id_image}" src="{$link->getImageLink($product->link_rewrite, $imageIds, 'medium_default')|escape:'html':'UTF-8'}" alt="{$imageTitle}" title="{$imageTitle}" {*}height="{$cartSize.height}" width="{$cartSize.width}"{*} itemprop="image" />
									</a>
								</li>
							{/foreach}
						{/if}
		...
		<div class="pb-right-column col-xs-12 col-sm-7 col-md-7">
			...
			<form id="buy_block"{if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0} class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
                ...
                <div id="box-product" class="col-sm-12">
                    {*hook h='displayOvicCategorySizeChart'*}
								...
								{foreach from=$groups key=id_attribute_group item=group}
									{if $group.attributes|@count}
										<fieldset class="attribute_fieldset">
											<label class="attribute_label{if $group.group_type == 'color'} label_color{/if}" {if $group.group_type != 'color' && $group.group_type != 'radio'}for="group_{$id_attribute_group|intval}"{/if}>{$group.name|escape:'html':'UTF-8'}&nbsp;</label>
                    ...
                    <div class="box-cart-bottom">
						...
						<p id="add_to_cart" class="buttons_bottom_block no-print {if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} unvisible{/if}">
							<button type="submit" name="Submit" class="exclusive">
								<span><i class="fa fa-shopping-cart"></i>{if $content_only && (isset($product->customization_required) && $product->customization_required)}{l s='Customize'}{else}{l s='Add to Cart'}{/if}</span>
							</button>
						</p>
						{if isset($HOOK_PRODUCT_ACTIONS) && $HOOK_PRODUCT_ACTIONS}{$HOOK_PRODUCT_ACTIONS}{/if}
                        <a class="add_to_compare" id="add_to_compare" href="javascript:void(0)" data-id-product="{$product->id|intval}" title="{l s='Add to compare'}"><i class="fa fa-toggle-on"></i></a>
     ...
     <div id="tab-container" class="col-xs-12">
        ...
        <ul id="product-tabs" class="nav nav-tabs" role="tablist">
            {if isset($product) && $product->description}
                <li class="active"><a href="#descripntion-tab" role="tab" data-toggle="tab">{l s='Product Details'}</a></li>{/if}
            {if isset($features) && $features}
                <li {if !isset($product->description)}class="active"{/if}><a href="#table-data-sheet" role="tab" data-toggle="tab">{l s='Information'}</a></li>{/if}
            {if (isset($quantity_discounts) && count($quantity_discounts) > 0)}
                <li {if !isset($product->description) && !isset($features)}class="active"{/if}><a href="#quantityDiscount-tab" role="tab" data-toggle="tab">{l s='Discounts'}</a></li>{/if}
            {if $HOOK_PRODUCT_TAB}
                {$HOOK_PRODUCT_TAB}
            {/if}
        </ul>
