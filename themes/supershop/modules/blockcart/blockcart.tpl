...
{/if}
	<div class="shopping_cart">
		<a href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
			<span class="cart_name_block">{l s='Cart' mod='blockcart'}</span><br />
			...
			<span class="ajax_cart_no_product{if $cart_qties > 0} unvisible{/if}">
                <span>{l s='0 Item(s)' mod='blockcart'}</span>
			...
			<div class="cart_block block exclusive ">
				...
				<div class="block_content mCustomScrollbar">
					...
					<div class="cart_block_list{if isset($blockcart_top) && !$blockcart_top}{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)} expanded{else} collapsed unvisible{/if}{/if}">
						{if $products}
							<span class="ajax_cart_quantity{if $cart_qties == 0} unvisible{/if}">{$cart_qties}</span>
							<span class="ajax_cart_product_txt{if $cart_qties != 1} unvisible{/if}">{l s=' ITEM IN MY CART' mod='blockcart'}</span>
							<span class="ajax_cart_product_txt_s{if $cart_qties < 2} unvisible{/if}">{l s=' ITEMS IN MY CART' mod='blockcart'}</span>
							
							<dl class="products">								
								{foreach from=$products item='product' name='myLoop'}
									{assign var='productId' value=$product.id_product}
									{assign var='productAttributeId' value=$product.id_product_attribute}
									<dt data-id="cart_block_product_{$product.id_product}_{if $product.id_product_attribute}{$product.id_product_attribute}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery}{else}0{/if}" class="cartblock-item-{$product.id_product}_{if $product.id_product_attribute}{$product.id_product_attribute}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery}{else}0{/if} cleafix mainHoverColor {if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
										<div class="pull-left">
											<span class="remove_link">
												{if !isset($customizedDatas.$productId.$productAttributeId) && (!isset($product.is_gift) || !$product.is_gift)}
													<a class="ajax_cart_block_remove_link" href="{$link->getPageLink('cart', true, NULL, 'delete=1&amp;id_product={$product.id_product}&amp;ipa={$product.id_product_attribute}&amp;id_address_delivery={$product.id_address_delivery}&amp;token={$static_token}', true)|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='remove this product from my cart' mod='blockcart'}"><i style="font-size: 22px;" class="icon-trash"></i></a>
												{/if}
											</span>
										...
										<div class="cart-info pull-left">
											<div class="product-name">                                                
												<a class="cart_block_product_name" href="{$link->getProductLink($product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}">{$product.name|truncate:22:'...'|escape:'html':'UTF-8'}</a>
											</div>
											...
											<span class="price">
												...
												<div style="width: 150px" class="change_quantity">
												  <a onclick="blockcartDownQuantity('{$product.id_product}_{if $product.id_product_attribute}{$product.id_product_attribute}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery}{else}0{/if}', '')" class="blockcart_quantity_down" 
												  id="blockcart_quantity_down_{$product.id_product}_{if $product.id_product_attribute}{$product.id_product_attribute}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery}{else}0{/if}" 
												  href="javascript:void(0)" rel="nofollow" title="{l s='Subtract'}"><i class="icon-minus"></i></a>
												  <input type="hidden" name="blockcart_quantity_{$product.id_product}_{if $product.id_product_attribute}{$product.id_product_attribute}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery}{else}0{/if}_hidden" value="{$product.cart_quantity}" class="cart_quantity_input_text" />
												  <input type="text" name="blockcart_quantity_{$product.id_product}_{if $product.id_product_attribute}{$product.id_product_attribute}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery}{else}0{/if}" class="cart_quantity_input_text" value="{$product.cart_quantity}">
												  <a onclick="blockcartUpQuantity('{$product.id_product}_{if $product.id_product_attribute}{$product.id_product_attribute}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery}{else}0{/if}', '')" class="blockcart_quantity_up" 
												  id="blockcart_quantity_up_{$product.id_product}_{if $product.id_product_attribute}{$product.id_product_attribute}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery}{else}0{/if}"	
												  href="javascript:void(0)" rel="nofollow" title="{l s='Add'}"><i class="icon-plus"></i></a>
												</div>
						...
						<div class="cart-prices">
							...
							<div class="cart-prices-line last-line">
                                ...
                                <span class="price cart_block_total ajax_block_cart_total pull-right">{$total}</span>
							</div>
						...
						<p class="cart-buttons">
                            ...
                            <a class="btn btn-default button button-small mainBorder" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">
								<span class="">
									{l s='View My Cart' mod='blockcart'}
								</span>
							</a>
...
{/if}
<script type="text/javascript" src="{$js_dir}jquery.mCustomScrollbar.concat.min.js"></script>
{strip}
